...
.PHONY: release
release:
	sed -i "/<releases>/a \    <release version=\"$$($(SEMVER) get release)\" date=\"$$(date +%F)\">\n      <url>https://github.com/skynomads/orchestrator/releases/tag/$$($(SEMVER) get release)</url>\n    </release>" dev.silkepilon.Orchestrator.appdata.xml
	git add .semver.yaml dev.silkepilon.Orchestrator.appdata.xml
